    ...
    public class Repository
        {
            ...
            public Repository()
            {
                client = new GraphClient(new Uri("http://localhost:7474/db/data"), "neo4j", "Anc!45n4p!5");
                client.Connect();
            }
            ...
            public IEnumerable<User> PickRandomUsers(int number, int max)
            {
            ...
            public IEnumerable<PathsResult<User>> GetShortestPaths(int u1, int u2, int cid)
            {
                var pathsQuery =
                    client.Cypher
                    .Match("p = shortestPath((u:User)-[t:Trusts*..5]-(v:User)) WHERE(u.iduser = " + u1 + ") AND(v.iduser =  " + u2 + ") AND all(x IN rels(p) WHERE x.Category =  " + cid + ")")
            ...
            client
                    .Cypher
                    .Create(("(user:User{iduser:{id}, l1:{l1}, l2:{l2}, l3:{l3}, l4:{l4}, l5:{l5}})"))
                    .WithParam("id", i)
                    .WithParam("l1", l1)
                    .WithParam("l2", l2)
                    .WithParam("l3", l3)
            ...
            public void CreateProducts(int number)
            {
                ...
                client
                    .Cypher
                    .Create(("(prod:Product{idproduct:{id}, category:{category}, c1:{c1}, c2:{c2}, c3:{c3}, c4:{c4}, c5:{c5}})"))
                    .WithParam("id", i)
